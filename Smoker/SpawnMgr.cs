...
namespace Progetto6_Smoker_
{
    static class SpawnMgr
    {
        ...
        private static uint enemiesCounter;
        ...
        public static void Init()
        {
            ...
            for (int i = 0; i < enemies.Length; i++)
            {
                ...
                for (int j = 0; j < numEnemies; j++)
                {
                    ...
                    {
                        case 0:
                            enemies[i].Enqueue(new FollowEnemy(Vector2.Zero));
                            break;
                        case 1:
                            enemies[i].Enqueue(new ShootingEnemy(Vector2.Zero));
                            break;
                    }
        ...
        public static void Update()
        {
            ...
            if (enemiesSpawnCounter <= 0)
            {
                ...
                if (enemies[enemyType].Count > 0)
                {
                    ...
                    Vector2 randomPos = new Vector2(CameraMgr.MainCamera.position.X + CameraMgr.MainCamera.pivot.X + e.Width*0.5f, randomYpos);
            ...
            if (powerUpsSpawnCounter <= 0)
            {
                ...
                if (powerUps.Count > 0)
                {
                    ...
                    p.Reset();
                    p.Position = new Vector2(CameraMgr.MainCamera.position.X + CameraMgr.MainCamera.pivot.X + p.Width * 0.5f, Game.GetRandomScreenY((int)(p.Height * 0.5f)));
                }
            }
        }
